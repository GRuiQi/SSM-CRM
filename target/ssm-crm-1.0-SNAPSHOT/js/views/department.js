$(function () {
    ...
    var depDatagrid, depDatagridEditAndDel, depDiglog, depForm;
    depDatagrid = $("#dep_datagrid");
    depDatagridEditAndDel = $("#dep_datagrid_del,#dep_datagrid_edit");
    depDiglog = $("#dep_dialog");
    depForm = $("#dep_form");

    depDatagrid.datagrid({
        url: "/ssm-crm/dep_list",
        fit: true,
        fitColumns: true,
        rownumbers: true,
        pagination: true,
    ...
    var cmdObj = {
        ...
        add: function () {
            depDiglog.dialog("open");
            depDiglog.dialog("setTitle", "新增");
            depForm.form("clear");
        },
        ...
        save: function () {
            ...
            // 如果有id，说明是编辑；否则，是保存。
            if (idVal) {
                url = "/ssm-crm/dep_update";
            } else {
                url = "/ssm-crm/dep_save";
            }
        ...
        edit: function () {
            ...
            var rowData = depDatagrid.datagrid("getSelected");
            // 这里rowData不是boolean，但是只要非false都是true。感觉这样写其实不好，最好是判断是否为空啥的。
            if (rowData) {
                depDiglog.dialog("open");
                depDiglog.dialog("setTitle", "编辑");
                depForm.form("clear");
                depForm.form("load", rowData);//把rowData加载到弹窗显示
            } else {
        ...
        del: function () {
            ...
            if (rowData) {
                ...
                $.messager.confirm("温馨提示", "您确定要删除这条数据吗？", function (yes) {
                    if (yes) {
                        $.get("/dep_delete?id=" + rowData.id, function (data) {
                            if (data.success) {
                                $.messager.alert("温馨提示", data.msg, "info", function () {
                                    // 刷新表格数据
                                    depDatagrid.datagrid("reload");
